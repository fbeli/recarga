 ...
 */
	 ...
	 * @throws ErroException
	 */
	public List<Recarga> buscaTransacoesPendentes(Usuario user) throws ErroException {
		
		List<Recarga> recargas = buscaTransacoesPendentes();
		
		for (Recarga recarga : recargas) {
			if(recarga.getUsuarioLocal().equals(user.getNome()))
	...
	public Recarga cancelarRecarga(Recarga recarga) throws ErroException{
		
		recarga = rs.confirmarRecarga(recarga, "1");
		
		return recarga;
	}
	public List<Recarga> cancelarRecargas(List<Recarga> recargas) throws ErroException{
		
		for (Recarga recarga: recargas) {
			rs.confirmarRecarga(recarga, "1");
		}			
		return recargas;
			
	}
	public String cancelarRecargasPendentes() throws ErroException{
		
			StringBuffer sb = new StringBuffer();
			List<Recarga> recargas = cancelarRecargas(buscaTransacoesPendentes());
		
			for (Recarga recarga : recargas) {
				sb.append("Recarga: '"+recarga.getCodOnline()+"'");
				sb.append("Status da recarga no servidor: '"+recarga.getStatusRecargaServer()+"'");
				sb.append("Status da recarga: "+recarga.getStatusRecarga()+"'\n");
			}
