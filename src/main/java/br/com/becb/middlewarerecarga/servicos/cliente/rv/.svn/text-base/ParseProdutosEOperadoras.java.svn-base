...
public class ParseProdutosEOperadoras {
	...
	private List<Operadora> createOperadoras(CellCard cellcard) {
		...
		if (cellcard.getOperadoras() != null
			...
			Logar.info("Desativando Operadoras : "+Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
	...
	private Operadora createOperadoras(OperadoraRV oRV) throws Exception {
		...
		List<Produto> produtos = createProdutos(oRV.getProdutos()
				.getProdutosRV());
		Logar.info("Salvando Operadora: "+op.getNomeOperadora()+ "  "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
		...
		try {
			...
			for (int i = 0; i < produtos.size(); i++) {
				...
				Logar.info("Associa Produto e Operadora INICIO - ParseProdutoEOperadora 171   ");
				
				op.setProduto(this.associaProdutoeOperadora(prod, op));
				fim = Calendar.getInstance().getTime();
				Logar.info("Associa Produto e Operadora FIM - ParseProdutoEOperadora 171 -  "+ inicio.compareTo(fim));
				inicio = Calendar.getInstance().getTime();
				Logar.info("Salvar Produto INICIO - ParseProdutoEOperadora 171 -  "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
				
				this.salvarProduto(prod);
				fim = Calendar.getInstance().getTime();
				Logar.info("Produto Salvo FIM - ParseProdutoEOperadora 175 -  "+ inicio.compareTo(fim)+" "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
				Logar.info("Associar Operadora e Produto INICIO - ParseProdutoEOperadora 176 -  "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
				
				jdbcDaoProduto.acertarAssociacaoOperadoraeProduto(prod.getId(),
						op.getId());
				Logar.info("Associar Operadora e Produto FIM - ParseProdutoEOperadora 180 -  "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
	...
	private Operadora salvarOperadora(Operadora operadora) {
		 ...
		 * associação
		 */
		Logar.info("salvar operadora - ParseProdutoseOperadora 269 - "+ Suporte.conveterData(Calendar.getInstance().getTime(), "YYYY-MM-DD hh:mm:ss"));
